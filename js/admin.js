...
/***** SHOP ******/
//페이지가 생성될 때 한번 실행되며 루트의 shop레퍼런스에 콜백을 링크한다.
function initShop() {
	$(".grid > ul").remove();
...
function shopDel(obj){
	var id =$(obj).parent().parent().parent().attr("id");
	console.log(id);
	if (confirm("이거 지우면 다 지워져요\n1차카테고리 삭제시 하위카테고리도 삭제됩니다.")){
...
function shopDel2(obj){
    ...
    var idUl = $(obj).parent().parent().parent().attr("id");
    var idli = $(obj).parent().parent().attr("id");

	if(confirm("정말삭제?")){
	   db.ref("root/shop/" + idUl + "/sub/" + idli ).remove();
...
function shopUp2(obj){
	...
	var title = $(".title", $(obj).parent().parent()).val();
	var icon = $(".icon",  $(obj).parent().parent()).val();
	var color = $(".color",  $(obj).parent().parent()).val();
	var link = $(".link",  $(obj).parent().parent()).val();
	if (title == '' || icon == '' || color == '' || link == '') {
		$(".title", $(obj).parent().parent()).focus();
		alert("내용을 적어주세요.");
	} else {
		db.ref("root/shop/" +idUl+ "/sub/" + idLi).update({
			title: title,
			icon: icon,
			color: color,
			link: link
	...
	var idUl = $(obj).parent().parent().parent().attr("id");
	var idli = $(obj).parent().parent().attr("id");
	var idval= $(obj).parent().parent().val();

	ref = db.ref("root/shop/" +idUl+ "/" + idli);
	ref.once("value").then(function(snapshot){
		idval.val(snapshot.val());
	})
*/
}
...
/**참조사항 
 * || : or 연산자 (또는) => ture || true(true)  / true||false(true) / flase || flase(flase)
 * && : and 연산자 (그리고) => ture || true(true)  / true||false(false) / flase || flase(flase)
 * if
 */

/*
$("#nav-home").css("background-color","gray");

$("#nav-home").click(function(){
    $("#nav-home").css("background-color","gray");
    $("#nav-shop").css("background-color","white");
    $("#nav-blog").css("background-color","white");
})

$("#nav-shop").click(function(){
    $("#nav-shop").css("background-color","gray");
    $("#nav-home").css("background-color","white");
    $("#nav-blog").css("background-color","white");
})
$("#nav-blog").click(function(){
    $("#nav-blog").css("background-color","gray");
    $("#nav-shop").css("background-color","white");
})
/*
$("#nav-blog").click(function(){
